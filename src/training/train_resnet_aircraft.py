...
- Optimisé pour GPU (CUDA) + mixed precision (AMP)
"""
...
BATCH_SIZE = 64
NUM_EPOCHS = 40            # tu peux monter si ça tourne bien
VAL_RATIO = 0.2            # 20% du dataset pour la validation
LEARNING_RATE = 1e-3
NUM_WORKERS = 4            # adapter selon ton CPU
SEED = 42
...
def get_device():
    if torch.cuda.is_available():
        ...
        print(f"✅ Utilisation de CUDA : {torch.cuda.get_device_name(0)}")
        # optimise les convolutions pour la taille de batch / input
...
def main():
    ...
    # Mixed precision seulement si on est sur CUDA
