...
# =========================
# Cohérence avion / cocarde
# =========================

# À COMPLÉTER avec tes 95 classes avions
# clé = label avion (top-1 ResNet), valeur = liste de pays possibles (labels cocardes)
AIRCRAFT_TO_COUNTRIES = {
    "A10":      ["usa"],
    "A400M":    ["france", "germany", "uk", "turkey"],
    "AG600":    ["china"],
    "AH64":     ["usa", "uk", "israel", "japan", "egypt"],
    "AKINCI":   ["turkey"],
    "AV8B":     ["usa", "italy"],

    "An124":    ["russia", "ukraine"],
    "An22":     ["russia", "ukraine"],
    "An225":    ["ukraine"],
    "An72":     ["russia", "ukraine"],
    ...
    "C1":       ["japan"],
    "C130":     ["usa", "uk", "france", "australia", "egypt",
                 "israel", "india", "italy", "japan", "south_korea"],
    "C17":      ["usa", "uk", "india", "australia"],
    "C2":       ["japan"],
    "C390":     [],   # opérateurs hors de tes 16 pays → pas de contrainte

    "C5":       ["usa"],

    "CH47":     ["usa", "uk", "italy", "japan"],
    "CH53":     ["usa", "germany", "israel"],

    "CL415":    ["france", "italy"],

    "E2":       ["usa", "france", "japan", "egypt"],
    "E7":       ["australia", "turkey", "uk"],

    "EF2000":   ["uk", "germany", "italy"],

    "EMB314":   ["usa"],  # Super Tucano (utilisation US dans ton contexte possible)

    "F117":     ["usa"],
    "F14":      ["usa"],
    "F15":      ["usa", "japan", "israel"],
    "F16":      ["usa", "turkey", "egypt", "israel"],
    "F18":      ["usa", "australia"],
    "F2":       ["japan"],
    "F22":      ["usa"],
    "F35":      ["usa", "uk", "italy", "japan"],
    "F4":       ["usa", "germany", "japan", "israel"],

    "FCK1":     [],   # Taïwan → hors de ta liste

    "H6":       ["china"],

    "Il76":     ["russia", "india", "china"],
    ...
    "JAS39":    [],   # Suède / autres → hors de ta liste

    "JF17":     ["china"],  # co-développé avec le Pakistan (non listé)

    "JH7":      ["china"],

    "KAAN":     ["turkey"],

    "KC135":    ["usa", "france", "turkey"],
    ...
    "MQ25":     ["usa"],
    "MQ9":      ["usa", "uk", "france", "italy"],

    "Mi24":     ["russia", "ukraine", "india", "egypt"],
    "Mi26":     ["russia", "india"],
    "Mi28":     ["russia"],
    "Mi8":      ["russia", "ukraine", "egypt", "india"],

    "Mig29":    ["russia", "ukraine", "india"],
    "Mig31":    ["russia"],

    "Mirage2000": ["france", "india", "egypt"],

    "P3":       ["usa", "japan", "germany", "australia"],
    "RQ4":      ["usa", "japan", "germany"],

    "Rafale":   ["france", "india", "egypt"],
...
def get_allowed_countries_for_aircraft(aircraft_label: Optional[str]) -> Optional[list]:
    """Retourne la liste de pays autorisés pour un avion donné, ou None si inconnu."""
    if aircraft_label is None:
        return None
    return AIRCRAFT_TO_COUNTRIES.get(aircraft_label)


# ============================================================
# Config portable (relative au projet)
# ============================================================

# Ce fichier est dans src/main.py → on remonte d’un niveau pour la racine
ROOT_DIR = Path(__file__).resolve().parents[1]

MODELS_DIR = ROOT_DIR / "models"
DATA_DIR = ROOT_DIR / "data"

# Modèles (à adapter si tu changes de noms)
MODEL_YOLO_PATH = MODELS_DIR / "yolo" / "yolov8n.pt"
MODEL_RESNET_PATH = MODELS_DIR / "resnet" / "resnet18_military_aircraft_best.pth"

# NEW: modèle YOLO pour les cocardes
MODEL_COCARDE_PATH = MODELS_DIR / "yolo" / "cocardes_merged_v2_best.pt"  # à adapter

# Dossier d’entraînement prioritaire (un sous-dossier par classe)
# ex : data/raw/crop/A10, data/raw/crop/F16, etc.
TRAIN_DIR = DATA_DIR / "raw" / "crop"
...
# NEW: Seuils YOLO cocardes
COCARDE_CONF_THRES = 0.6
COCARDE_IOU_THRES = 0.5
COCARDE_IMAGE_SIZE = 640
...
def get_device_labels():
    ...
    device_resnet = "mps" if torch.backends.mps.is_available() else (
...
def ensure_exists(path: Union[str, Path], kind: str = "fichier"):
...
# ============================================================
def discover_classes_from_dir(root: Union[str, Path],
                              min_images: int = 1,
                              natural_sort: bool = False) -> List[str]:
    root = str(root)
...
@timeit
def load_resnet_autodetect(model_path: Union[str, Path],
    ...
    if classes_in_ckpt is not None and isinstance(classes_in_ckpt, (list, tuple)):
        if classes is None:
            ...
            log(
                f"⚠️ Liste de classes fournie ({len(classes)}) ≠ checkpoint ({len(classes_in_ckpt)}). "
    ...
    if "fc.weight" in state_dict:
        ...
        if ckpt_classes > model_classes:
            # Le checkpoint a PLUS de classes que le modèle → on tronque
            print(
                f"⚠️ Ajustement du FC par tronquage : "
                f"checkpoint={ckpt_classes} classes, modèle={model_classes} classes"
            )
            state_dict["fc.weight"] = state_dict["fc.weight"][:model_classes]
            state_dict["fc.bias"] = state_dict["fc.bias"][:model_classes]

        elif ckpt_classes < model_classes:
            # Là seulement on lève l'erreur
            raise ValueError(
                "Checkpoint a moins de classes que le modèle — cas non supporté."
            )
        # Si ckpt_classes == model_classes → on ne fait rien, c’est parfait.
...
# ============================================================
@timeit
def load_yolo(model_path: Union[str, Path], device: str = "cpu"):
    if YOLO is None:
...
# NEW: YOLO sans filtrage (utilisé pour les cocardes)
def yolo_detect_raw(yolo_model,
                    image: Union[str, Image.Image],
                    conf: float,
                    iou: float,
                    imgsz: int = YOLO_IMAGE_SIZE,
...
def analyser_image(image: Union[str, Image.Image, None],
                   ...
                   topk: int = 3,
                   imgsz: int = YOLO_IMAGE_SIZE,
                   cocarde_model=None,            # NEW
                   cocarde_names=None):           # NEW
    ...
    # === Candidats cocardes pour chaque crop avion ===
    cocarde_candidates_per_crop = []
    if cocarde_model is not None:
        for det, crop in zip(detections, crops):
            coc_dets = yolo_detect_raw(
                cocarde_model,
                crop,
                conf=0.001,  # garder un max de boxes
                iou=COCARDE_IOU_THRES,
                imgsz=COCARDE_IMAGE_SIZE,
                tta=False,
                agnostic_nms=True,
                max_det=50,
            )
            if not coc_dets:
                cocarde_candidates_per_crop.append([])
                continue
            ...
            for d in coc_dets:
                ...
                if isinstance(cocarde_names, dict):
                    flag_label = cocarde_names.get(cid, f"flag_{cid}")
                elif isinstance(cocarde_names, (list, tuple)) and 0 <= cid < len(cocarde_names):
                    flag_label = cocarde_names[cid]
                else:
                    flag_label = f"flag_{cid}"

                candidates.append({
                    "label": flag_label,
                    "score": d["score"],
                    "cls_id": cid,
                    "bbox_local": (cx1, cy1, cx2, cy2),
                    "bbox_global": (gx1, gy1, gx2, gy2),
                })

            # tri par score décroissant
            candidates.sort(key=lambda c: c["score"], reverse=True)
            cocarde_candidates_per_crop.append(candidates)
    else:
        cocarde_candidates_per_crop = [[] for _ in crops]

    # === Sélection finale avec vérif de cohérence ===
    outputs = []
    for det, topk_list, candidates in zip(detections, topk_per_crop, cocarde_candidates_per_crop):
        aircraft_label = topk_list[0]["label"] if topk_list else None
        allowed_countries = get_allowed_countries_for_aircraft(aircraft_label)

        chosen_cocarde = None
        cocarde_corrigee = False
        cocarde_incoherente = False

        if candidates:
            if not allowed_countries:
                # Aucun mapping connu → on prend simplement la meilleure
                chosen_cocarde = candidates[0]
            else:
                # On teste successivement la 1ère, 2ème, 3ème, ...
                for idx, cand in enumerate(candidates):
                    if cand["label"] in allowed_countries:
                        chosen_cocarde = cand
                        cocarde_corrigee = (idx > 0)
                        break

                # Si aucune cocarde cohérente trouvée → on garde la meilleure brute
                if chosen_cocarde is None:
                    chosen_cocarde = candidates[0]
                    cocarde_incoherente = True

        outputs.append({
            "coordonnees": det["bbox"],
            "confiance_detection": det["score"],
            "topk": topk_list,
            "cocarde": chosen_cocarde,
            "cocarde_corrigee": cocarde_corrigee,
            "cocarde_incoherente": cocarde_incoherente,
        })
...
def draw_annotations(img: Image.Image, outputs: List[Dict[str, Any]]) -> Tuple[Image.Image, Optional[Image.Image]]:
    ...
    for i, out in enumerate(outputs):
        ...
        coc = out.get("cocarde")
        corrigee = out.get("cocarde_corrigee", False)
        incoh = out.get("cocarde_incoherente", False)
        ...
        extra = ""
        # Cocarde (rouge)
        if coc is not None and "bbox_global" in coc:
            fx1, fy1, fx2, fy2 = coc["bbox_global"]
            draw.rectangle([fx1, fy1, fx2, fy2], outline=(255, 0, 0), width=2)

            flag_label = coc["label"]
            flag_score = coc["score"]
            extra = f" | flag {flag_label} {flag_score:.2f}"
            if corrigee:
                extra += " (corrigée)"
            elif incoh:
                extra += " (incohérente)"

        text = f"{label} {p:.2f} | det {score:.2f}{extra}"
...
def format_results_text(outputs: List[Dict[str, Any]]) -> str:
    ...
    for i, r in enumerate(outputs, 1):
        ...
        coc = r.get("cocarde")
        if coc is not None:
            corrigee = r.get("cocarde_corrigee", False)
            incoh = r.get("cocarde_incoherente", False)
            suffix = ""
            if corrigee:
                suffix = " (corrigée)"
            elif incoh:
                suffix = " (incohérente)"
            lines.append(
                f"   🌍 Cocarde: {coc['label']} — conf={coc['score']:.4f} (id={coc['cls_id']}){suffix}"
            )
    return "\n".join(lines)
...
def build_runtime():
    ...
    if DISCOVER_CLASSES_FROM_DIR:
        classes = discover_classes_from_dir(
            ...
            NATURAL_SORT_CLASSES
        )
        if classes:
            log(f"📁 Classes découvertes ({len(classes)}): "
                f"{classes[:]}{' ...' if len(classes) > 10 else ''}")
        else:
    ...
    # NEW: chargement modèle cocardes
    cocarde_model = None
    cocarde_names = None
    try:
        cocarde_model = load_yolo(MODEL_COCARDE_PATH, device=device_yolo)
        cocarde_names = cocarde_model.model.names if hasattr(cocarde_model, "model") else cocarde_model.names
        log(f"🎯 Modèle cocardes chargé ({len(cocarde_names)} classes)")
    except Exception as e:
        log(f"⚠️ Impossible de charger le modèle cocardes: {e}")
    ...
    # NEW: on retourne aussi le modèle cocardes + noms de classes
    return yolo_model, resnet_model, classes, device_resnet, cocarde_model, cocarde_names
...
def predict(pil_img, conf, iou, topk, imgsz):
    ...
    # NEW: on récupère aussi cocarde_model + cocarde_names
    yolo_model, resnet_model, classes, device_resnet, cocarde_model, cocarde_names = predict._rt
    ...
    outputs, annotated, first_crop = analyser_image(
        ...
        conf=conf, iou=iou, resnet_img_size=RESNET_INPUT_SIZE,
        topk=topk, imgsz=imgsz,
        cocarde_model=cocarde_model, cocarde_names=cocarde_names  # NEW
    )
...
def run_cli(args):
    yolo_model, resnet_model, classes, device_resnet, cocarde_model, cocarde_names = build_runtime()
    outputs, annotated, first_crop = analyser_image(
        args.image, yolo_model, resnet_model, classes, device_resnet,
        conf=args.conf, iou=args.iou, resnet_img_size=RESNET_INPUT_SIZE,
        topk=args.topk, imgsz=args.imgsz,
        cocarde_model=cocarde_model, cocarde_names=cocarde_names  # NEW
    )
    ...
    with gr.Blocks(title="YOLO + ResNet") as demo:
        with gr.Row():
            with gr.Column():
                ...
                conf = gr.Slider(0.0, 1.0, value=YOLO_CONF_THRES,
                                 step=0.01, label="Confiance YOLO")
                iou = gr.Slider(0.0, 1.0, value=YOLO_IOU_THRES,
                                step=0.01, label="IoU YOLO")
                imgsz = gr.Slider(320, 1536, value=YOLO_IMAGE_SIZE,
                                  step=32, label="Taille d’inférence YOLO")
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Détection (YOLO) + classification (ResNet) Top-k"
    )
    ap.add_argument("--image", type=str,
                    help="Chemin d'une image pour la version CLI")
    ap.add_argument("--conf", type=float, default=YOLO_CONF_THRES,
                    help="Seuil confiance YOLO")
    ap.add_argument("--iou", type=float, default=YOLO_IOU_THRES,
                    help="Seuil IoU YOLO")
    ap.add_argument("--imgsz", type=int, default=YOLO_IMAGE_SIZE,
                    help="Taille d’inférence YOLO")
    ap.add_argument("--topk", type=int, default=3,
                    help="Top-k pour ResNet")
    ap.add_argument("--save", type=str, default="",
                    help="Chemin de sortie pour l'image annotée (CLI)")
...
def main():
    ...
    if args.gradio:
        if not _HAS_GRADIO:
            print("Gradio non installé. Faites: pip install gradio",
                  file=sys.stderr)
            sys.exit(2)
        demo = build_ui()
